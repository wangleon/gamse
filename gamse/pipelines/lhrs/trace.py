...
from ..espadons.common import get_mean_profile

def norm_profile(xnodes, ynodes):
    A = ynodes.max()
    c = np.mean(xnodes)
    ynodes = ynodes/A
    xnodes = xnodes - c
    param = (A, c)
    return xnodes, ynodes, param


def find_order_locations(section, x, aligned_allx=None, mode='normal'):
    ...
    if mode=='debug':
        ...
        figname_order_width = os.path.join(dbgpath,
                            'order_width_{:04d}.png'.format(x))
        figname_multi_peak = lambda multi_order: os.path.join(dbgpath,
                            'multi_{:04d}_{:02d}.png'.format(x, multi_order))
        figname_section    = os.path.join(dbgpath,
                            'section_{:04d}.png'.format(x))
    else:
    ...
    min_width = 10 # minimum width of order

    xnodes = [100, 2000, 3800]
    wnodes = [50,  86, 125]       # lengths of order distance in xnodes
    snodes = [4,  11, 26]        # lengths of order gaps in xnodes

    c1 = np.polyfit(xnodes, wnodes, deg=2)
    c2 = np.polyfit(xnodes, snodes, deg=2)
    ...
    for i1 in np.arange(0, nx, 1):
        ...
        if i2 >= nx-1:
            break
        v = np.percentile(section[i1:i2], min(100, percent))
    ...
    if x == 2148:
        fig = plt.figure()
        ax = fig.gca()
        ax.plot(np.log(section))
        ax.plot(allx[winmask], np.log(section[winmask]), 'o', ms=3)
        ax.plot(newy)
        ax.plot(newy+3*std)
        fig.savefig('x-{}.png'.format(x))
        plt.show()
    ...
    if plot_order_width:
        ...
        title = 'Order Widths and Separations for Column {:04d}'.format(x)
        figw.suptitle(title)
    ...
    for multi_order in multi_order_lst:
        ...
        message = ('Col {:4d} - Multi order: {:2d} {:4d}-{:4d}. width={:4d}  '
             'Multi={:4.2f}').format(x, multi_order, i1, i2, i2-i1, multi)
        logger.info(message)
        print(message)
        ...
        if plot_multi_peak:
            ax.set_xlim(i1, i2)
            ax.set_ylim(_y1, _y2)
            title = 'Multi-peak detected in Column {:04d}'.format(x)
            fig.suptitle(title)
    ...
    if plot_section:
        ...
        title = 'Cross section of Column {:04d}'.format(x)
        fig2.suptitle(title)
    ...
    result_lst = [(i1, i2) for i1, i2 in order_index_lst]
    return result_lst

def find_apertures(data, mask, scan_step=100, minimum=10, separation=None,
        align_deg=2, conv_core=10, filling=0.3, degree=4, display=False,
        mode='normal', figpath='images', fig=None,
        ):
    ...
    while(True):
        ...
        flux1 = np.mean(data[:, x1-2:x1+3], axis=1)
